    ...
    public partial class MainWindow : Window
    {
        ...
        private Task Client_ApplicationMessageReceivedAsync(MqttApplicationMessageReceivedEventArgs arg)
        {
            ...
            if (message.Substring(0, 1) == "{" && message.Substring(message.Length - 1) == "}")
            {
                try
                {
                    ...
                    {
                        if (Logging)
                            File.AppendAllText("Log_" + time + ".txt", $"{user["Time"].GetString()}, {user["ValueS"].GetString()} , {user["Range"].GetString()}" + System.Environment.NewLine);
                    }
                }
                catch (Exception ex)
        ...
        private void button_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (Logging)
            {
                ...
                File.AppendAllText("Log_" + time + ".txt", "Time, Value, Range" + System.Environment.NewLine);
